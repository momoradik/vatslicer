...
public class ComprehensiveModelAnalyzerTests
{
    ...
    [Fact]
    public void SimpleBox_RunsAllChecks()
    {
        var mesh = BuildBox(-10, -10, 0, 10, 10, 20);
        var report = ComprehensiveModelAnalyzer.Analyze(mesh);
        report.TriangleCount.Should().Be(12);
        report.MeshValid.Should().BeTrue();
        report.OverallStatus.Should().NotBeNullOrEmpty();
        report.ElapsedMs.Should().BeGreaterOrEqualTo(0);
    }

    [Fact]
    public void Analyze_ReturnsVolumeAndArea()
    {
        var mesh = BuildBox(-5, -5, 0, 5, 5, 10);
        var report = ComprehensiveModelAnalyzer.Analyze(mesh);
        report.VolumeMm3.Should().NotBe(0);
        report.SurfaceAreaMm2.Should().BeGreaterThan(0);
    }
}
