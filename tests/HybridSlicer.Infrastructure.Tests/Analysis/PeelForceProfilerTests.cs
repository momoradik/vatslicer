...
public class PeelForceProfilerTests
{
    ...
    [Fact]
    public void SimpleBox_ProducesProfile()
    {
        var mesh = BuildBox(-10,-10,0,10,10,20);
        var profile = PeelForceProfiler.Compute(mesh, layerHeightMm: 1f);
        profile.Layers.Should().NotBeEmpty();
        profile.MaxPeelForceN.Should().BeGreaterThan(0);
        profile.AvgPeelForceN.Should().BeGreaterThan(0);
    }

    [Fact]
    public void MaxForce_AtWidestLayer()
    {
        var mesh = BuildBox(-10,-10,0,10,10,20);
        var profile = PeelForceProfiler.Compute(mesh, layerHeightMm: 1f);
        // Box has uniform area, so max force should be similar across layers
        profile.MaxPeelForceN.Should().BeGreaterThan(0);
    }
}
