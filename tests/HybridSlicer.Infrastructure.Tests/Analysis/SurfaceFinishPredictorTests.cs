...
public class SurfaceFinishPredictorTests
{
    ...
    [Fact]
    public void Box_HasMixedFinish()
    {
        var mesh = BuildBox(-10, -10, 0, 10, 10, 20);
        var report = SurfaceFinishPredictor.Predict(mesh);
        // Box has 2 horizontal faces (top/bottom) and 4 vertical faces
        report.SmoothPct.Should().BeGreaterThan(0);
        report.LayerLinePct.Should().BeGreaterThan(0);
        report.AvgQualityScore.Should().BeGreaterThan(0).And.BeLessThan(1);
        (report.SmoothPct + report.StairstepPct + report.LayerLinePct).Should().BeApproximately(100, 1);
    }
}
