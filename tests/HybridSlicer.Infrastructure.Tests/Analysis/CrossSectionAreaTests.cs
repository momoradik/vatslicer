...
public class CrossSectionAreaTests
{
    ...
    [Fact]
    public void Box_HasPositiveArea()
    {
        var mesh = BuildBox(-10, -10, 0, 10, 10, 20);
        var profile = CrossSectionAreaCalculator.Compute(mesh, 1f);
        profile.MaxAreaMm2.Should().BeGreaterThan(0);
        profile.TotalVolumeMm3.Should().BeGreaterThan(0);
        profile.LayerCount.Should().Be(20);
    }

    [Fact]
    public void Volume_RoughlyMatchesBox()
    {
        var mesh = BuildBox(0, 0, 0, 10, 10, 10);
        var profile = CrossSectionAreaCalculator.Compute(mesh, 0.5f);
        // Projected area approach overestimates (counts face area not cross-section)
        // but volume should be positive and in a reasonable range
        profile.TotalVolumeMm3.Should().BeGreaterThan(0f);
        profile.MaxAreaMm2.Should().BeGreaterThan(0f);
    }
}
