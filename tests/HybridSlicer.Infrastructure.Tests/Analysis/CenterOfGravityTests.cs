...
public class CenterOfGravityTests
{
    ...
    [Fact]
    public void SymmetricBox_CogAtCenter()
    {
        var mesh = BuildBox(-10, -10, 0, 10, 10, 20);
        var result = CenterOfGravityCalculator.Compute(mesh);
        result.CenterOfGravity.X.Should().BeApproximately(0, 1f);
        result.CenterOfGravity.Y.Should().BeApproximately(0, 1f);
        result.CenterOfGravity.Z.Should().BeApproximately(10, 2f);
        result.VolumeMm3.Should().BeGreaterThan(0);
    }

    [Fact]
    public void CogOffset_SmallForSymmetric()
    {
        var mesh = BuildBox(-5, -5, 0, 5, 5, 10);
        var result = CenterOfGravityCalculator.Compute(mesh);
        result.CogOffsetMm.Should().BeLessThan(3f, "symmetric box CoG should be near BB center");
    }
}
