...
public class ModelBoundsAnalyzerTests
{
    ...
    [Fact]
    public void SmallModel_FitsOnPlate()
    {
        var mesh = BuildBox(-5, -5, 0, 5, 5, 20);
        var r = ModelBoundsAnalyzer.Analyze(mesh, 192, 120, 200);
        r.FitsOnPlate.Should().BeTrue();
        r.FitsInHeight.Should().BeTrue();
    }

    [Fact]
    public void OversizedModel_DoesNotFit()
    {
        var mesh = BuildBox(-100, -100, 0, 100, 100, 300);
        var r = ModelBoundsAnalyzer.Analyze(mesh, 192, 120, 200);
        r.FitsOnPlate.Should().BeFalse();
        r.FitsInHeight.Should().BeFalse();
        r.MaxScaleToFit.Should().BeLessThan(1f);
    }

    [Fact]
    public void Suggestion_AlwaysProvided()
    {
        var mesh = BuildBox(0, 0, 0, 50, 50, 50);
        var r = ModelBoundsAnalyzer.Analyze(mesh, 192, 120, 200);
