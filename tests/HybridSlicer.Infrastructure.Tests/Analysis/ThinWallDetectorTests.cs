...
public class ThinWallDetectorTests
{
    private static (StlMesh mesh, AabbBvh bvh) BuildBox(float x1,float y1,float z1,float x2,float y2,float z2)
    {
        ...
        BitConverter.GetBytes(v[t*3+i].Z).CopyTo(d,o+8);o+=12;}o+=2;}
        var mesh = StlMesh.FromBinary(d);
        return (mesh, AabbBvh.Build(mesh));
    }

    [Fact]
    public void ThickBox_NoWarnings()
    {
        var (mesh, bvh) = BuildBox(-10, -10, 0, 10, 10, 20);
        var result = ThinWallDetector.Detect(bvh, mesh, minThicknessMm: 0.5f, sampleCount: 50);
        // A 20mm thick box should have no thin walls
        result.ThinWallCount.Should().Be(0);
    }

    [Fact]
    public void Detect_NoThrow()
    {
        var (mesh, bvh) = BuildBox(0, 0, 0, 5, 5, 5);
        var result = ThinWallDetector.Detect(bvh, mesh);
